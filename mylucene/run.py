...
def load_wemb(params, vocab):
    ...
    dim_emb_orig = list(wemb.values())[0].shape[0]

    W = 0.01 * np.random.randn(prm.n_words, dim_emb_orig).astype(config.floatX)
    for word, pos in list(vocab.items()):
        if word in wemb:
            W[pos,:] = wemb[word]
...
def build_model(tparams, options):
    ...
    if prm.l2reg > 0.:
        cost_l2reg = 0.
        for name, w in list(tparams.items()):
            #do not include bias.
...
def train():
    ...
    tparams_ = OrderedDict([(kk, vv) for kk, vv in tparams.items() if kk not in exclude_params])
    ...
    try:
        for eidx in range(prm.max_epochs):
            n_samples = 0
            ...
            for _, train_index in kf:
                ...
                if np.mod(uidx, prm.dispFreq) == 0:
                    ...
                    for ii in range(prm.n_iterations):
                        ...
                        print('  '.join(list(prm.metrics_map.keys())))
