...
import mylucene.parameters as prm
...
#
# def spacy_tokenize(text): # create a tokenizer function
#     # return [tok.text for tok in spacy_en.tokenizer(text)]
#     return [x.text for x in _remove_spaces(spacy_en(text))]
...
def BOW(words, vocab):
    ...
    bow_v = np.asarray(list(bow.values()))
    sumw = float(bow_v.sum())
    if sumw == 0.:
        sumw = 1.
    bow_v /= sumw

    return [list(bow.keys()), bow_v]
...
def load_vocab(path, n_words=None):
    ...
    if not n_words:
        n_words = len(list(dic.keys()))

    for i, word in enumerate(list(dic.keys())[:n_words]):
        vocab[word] = i
    return vocab
